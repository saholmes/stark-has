...
fn bench_e2e_mf_fri(c: &mut Criterion) {
    ...
    for &(label, schedule) in presets {
        ...
        for &k in &ks {
            ...
            // ---------------------------------------------------------
            // ✅ SYNTHETIC DEEP‑ALI INPUTS
            // ---------------------------------------------------------

            let a_eval: Vec<F> = (0..n0).map(|_| F::rand(&mut rng)).collect();
            let s_eval: Vec<F> = (0..n0).map(|_| F::rand(&mut rng)).collect();
            let e_eval: Vec<F> = (0..n0).map(|_| F::rand(&mut rng)).collect();
            let t_eval: Vec<F> = (0..n0).map(|_| F::rand(&mut rng)).collect();
            ...
            // ---------------- Prove ----------------
            let t0 = Instant::now();
            let proof = deep_fri_prove(f0_ali.clone(), domain0, &params);
            ...
            let proof_bytes = deep_fri_proof_size_bytes(&proof);
