...
impl<'a> MerkleProver<'a> {
    pub fn new(chan: &'a mut ProverChannel, cfg: MerkleChannelCfg, trace_hash: [u8; 32]) -> Self {
...
impl<'a> MerkleVerifier<'a> {
    ...
    pub fn verify_opening(&self, opening: &MerkleOpening, trace_hash: &[u8; 32]) -> bool {
        let root = match self.root {
            Some(r) => r,
            None => return false,
        };

        MerkleTreeChannel::verify_opening(&self.cfg, root, opening, trace_hash)
    }
