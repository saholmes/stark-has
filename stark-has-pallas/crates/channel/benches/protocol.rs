...
use channel::{
    MerkleChannelCfg, MerkleProver, MerkleVerifier, Mle, MleProver, MleVerifier, SumCheckMFConfig,
    SumCheckMFProver, SumCheckMFVerifier, SumCheckProver, SumCheckVerifier,
};
use transcript::Transcript;

// -------- Size helpers --------

fn field_len_bytes() -> usize {
    32
} // proxy for ark-pallas::Fr canonical encoding
        ...
        group.bench_with_input(BenchmarkId::new("prove_and_verify", k), &k, |b, &_k| {
            b.iter_batched(
                || {
                    ...
                    let table: Vec<F> = (0..n).map(|_| F::rand(&mut rng)).collect();
                    let mle = Mle::new(table.clone());
                    let mf_cfg = channel::SumCheckMFConfig {
...
criterion_group!(
    protocol_benches,
    bench_mle_commit_open,
    bench_sumcheck_plain,
    bench_sumcheck_mf
);
