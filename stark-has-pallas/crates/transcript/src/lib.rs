...
fn bytes_to_field_words(bytes: &[u8]) -> Vec<F> {
    let limb_bytes = <F as PrimeField>::BigInt::NUM_LIMBS * 8;
    bytes
        .chunks(limb_bytes)
...
mod blake3_backend {
    ...
    impl HashBackend for Blake3Backend {
        ...
        fn challenge(&mut self, label: &[u8]) -> F {
            ...
            h2.update(label);
            let out = h2.finalize();
            F::from_le_bytes_mod_order(out.as_bytes())
        }
    }
...
impl Transcript {
    ...
    pub fn with_backend(hash: FsHash, init_label: &[u8], params: poseidon::PoseidonParams) -> Self {
        emit_selected_backend_once();

        let backend: Box<dyn HashBackend> = match hash {
            FsHash::Poseidon => Box::new(poseidon_backend::Backend::new(params, init_label)),
            FsHash::Sha3_256 => Box::new(sha3_backend::Backend::new(init_label)),
            FsHash::Blake3 => Box::new(blake3_backend::Backend::new(init_label)),
        };
